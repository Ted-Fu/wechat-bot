...
export async function defaultMessage(msg, bot) {
  ...
  const sendTime = msg.date() // 发消息时间
  const msgAge = msg.age()  // 消息接收的时长，即当有人发送消息到机器人接收到消息之前的间隔，单位（seconds）
  const receiver = msg.to() // 消息接收人
  const content = msg.text() // 消息内容
  const isMentionSelf = await msg.mentionSelf() // 是否@自己
  ...
  const isRoom = roomWhiteList.includes(roomName) && isMentionSelf // 是否在群聊白名单内并且艾特了机器人
  const isAlias = aliasWhiteList.includes(remarkName) || aliasWhiteList.includes(name) // 发消息的人是否在联系人白名单内
  const isBotSelf = botName === remarkName || botName === name // 是否是机器人自己
  // TODO 你们可以根据自己的需求修改这里的逻辑
  if (isText && !isBotSelf && sendTime > serviceStartTime && msgAge < 180) {
    try {
      // 区分群聊和私聊
      if (isRoom && room) {
        await msg.say(await getReply(contact, content.replace(`@${botName}`, '')))
        return
      }
      // 私人聊天，白名单内的直接发送
      if (isAlias && !room) {
        await contact.say(await getReply(contact, content))
      }
    } catch (e) {
      console.error(e)
    }
